...
final class SkepProjectConfigTests: XCTestCase {
    func testParsesAllSupportedFields() async throws {
        ...
        let config = await SkepProjectConfig(projectPath: projectURL.path)
    ...
    func testMissingAndMalformedConfigReturnNilFields() async throws {
        ...
        let missingConfig = await SkepProjectConfig(projectPath: missingProjectURL.path)

        assertAllFieldsNil(in: missingConfig)

        let malformedProjectURL = try makeProjectDirectory()
        try Data("{not-json".utf8).write(to: malformedProjectURL.appendingPathComponent(".skep.json"))
        let malformedConfig = await SkepProjectConfig(projectPath: malformedProjectURL.path)
    ...
    private func writeConfig(_ jsonObject: [String: Any], to projectURL: URL) throws {
        ...
        try data.write(to: projectURL.appendingPathComponent(".skep.json"))
    }

    private func assertAllFieldsNil(in config: SkepProjectConfig) {
        XCTAssertNil(config.setupScript)
        XCTAssertNil(config.setupTimeoutSeconds)
        XCTAssertNil(config.teardownScript)
        XCTAssertNil(config.shellSetup)
