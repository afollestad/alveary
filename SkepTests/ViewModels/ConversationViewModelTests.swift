...
final class ConversationViewModelTests: XCTestCase {
    ...
    func testSetupAndStartCreatesWorktreeSpawnsAgentAndSendsFirstMessage() async throws {
        let worktreeInfo = WorktreeInfo(path: "/tmp/skep-worktree", branch: "skep/fix-auth")
        let fixture = try TestFixture(
...
private struct TestFixture {
    ...
    init(
        ...
        worktreeInfo: WorktreeInfo = WorktreeInfo(path: "/tmp/worktree", branch: "skep/thread")
    ) throws {
        ...
        let project = Project(path: "/tmp/skep-project", name: "Skep", remoteName: "origin", baseRef: "main")
        let thread = AgentThread(
            name: threadName,
