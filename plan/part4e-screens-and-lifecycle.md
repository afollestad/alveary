...
Phase 7 progress:
- [x] `AppDelegate` startup warmup now loads the session map, runs session-map-proven orphan cleanup, refreshes provider detection, and re-checks providers after wake with owned task cancellation.
- [x] Shutdown now begins runtime shutdown, emits `.appWillTerminate` before the blocking wait, terminates live agent processes, and performs the bounded best-effort `SessionManager.persist()` repair path.
- [x] Focused lifecycle coverage exists in `AppDelegateTests` for orphan cleanup ownership, wake-refresh cancellation, sudden-termination toggling, and orderly shutdown persistence.
- [ ] Signed-app notification delivery validation remains open in `plan/validation.md`.
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func cleanupOrphanedClaudeProcessesIfNeeded() async {
        ...
        //    the session-map entry for surviving conversations and pruning it when
        //    the SwiftData conversation record is already gone.
    }
```
...
| Yes | No | No | No | Terminate the orphan and remove the stale session-map entry |
