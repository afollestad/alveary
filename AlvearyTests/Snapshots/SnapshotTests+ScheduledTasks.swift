...
extension SnapshotTests {
    ...
    /// Drives the real `makeEditDraft` path rather than a hand-built draft, so the snapshot also
    /// proves a linked reuse thread reaches the row.
    func testScheduledTaskEditorReusedThreadNamesItsCreatedThread() throws {
        let fixture = try ScheduledTasksSnapshotFixture(includeTasks: false, includeReusedThreadTask: true)
        let draft = try XCTUnwrap(fixture.viewModel.makeEditDraft(definitionID: "reused-snapshot"))
        ...
        assertMacSnapshot(
            ...
            .padding(24),
            size: CGSize(width: 760, height: 400),
            named: "scheduled_task_editor_reused_thread_link"
        )
    }
