...
- **Run blocking test work through `blockingThreadTask`, never `Task.detached`,** when the test body waits on a semaphore it signals. `Support/BlockingThreadTask.swift` owns why: pool starvation on small CI runners, and priority inversions that fail `test.sh`.
