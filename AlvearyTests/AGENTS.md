...
- `assertMacSnapshot()` must build and tear down each host inside its autorelease pool, detach the hosting controller and content view, and close the window after comparison. Otherwise AppKit can retain SwiftUI's SwiftData observations until a later in-memory context save notifies those stale observers and traps inside SwiftData on macOS 26.
