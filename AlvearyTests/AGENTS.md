...
- **Never read a view's `@Environment` or `@FocusState` outside an installed body.** SwiftUI logs that as a runtime issue, which fails `test.sh`. Drive an unmounted view through a helper that takes the value as a parameter (`ChatView.harnessAuthenticationBanner`), or capture it from a hosted body (`Support/HostedFocusStateBinding.swift`).
- **Never block the main thread on a semaphore a detached task signals.** The Thread Performance Checker reports the priority inversion, which fails `test.sh`; give signalers `priority: .userInitiated` and keep blocking tests off the main actor, as the `Services/VoiceInput` tests do.
