...
- `assertMacSnapshot()` must build and tear down each host inside its autorelease pool, replace the hosted root with `EmptyView` while it is still window-attached, then detach the controller and content view and close the window. Detaching alone can leave SwiftUI's SwiftData observations registered until a later in-memory context save notifies those stale observers and traps inside SwiftData on macOS 26.
