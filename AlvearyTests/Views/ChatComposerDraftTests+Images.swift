...
extension ChatComposerDraftTests {
    ...
    func testStagedImagePreviewOpenPresentsInAppPreview() async throws {
        ...
        let preview = try XCTUnwrap(chatView.stagedComposerAttachments.first)
        guard case .image(let image) = preview else {
            return XCTFail("Expected an image attachment.")
        }
        chatView.openComposerAttachment(preview)
        ...
        if case .fileURL(let openedURL) = request.source {
            XCTAssertEqual(openedURL, image.fileURL.standardizedFileURL)
        } else {
    ...
    func testLocalImageSelectionStagesImagesWhenProviderDoesNotSupportAttachments() async throws {
        ...
        XCTAssertEqual(result, .handled)
        XCTAssertEqual(fixture.viewModel.stagedImageAttachments.first?.label, "picked.png")
    }

    func testLocalFileSelectionStagesNonImageFiles() async throws {
        let sourceDirectory = temporaryDirectory()
        defer {
            try? FileManager.default.removeItem(at: sourceDirectory)
        }
        let fileURL = sourceDirectory.appendingPathComponent("report.pdf")
        try Data("PDF".utf8).write(to: fileURL)

        let fixture = try ConversationViewModelTestFixture()
        let chatView = makeChatView(fixture: fixture, appState: AppState(), supportsLocalImageInput: true)

        let result = await chatView.handleLocalFileURLsSelected([fileURL])

        XCTAssertEqual(result, .handled)
        let attachment = try XCTUnwrap(fixture.viewModel.stagedFileAttachments.first)
        XCTAssertEqual(attachment.label, "report.pdf")
