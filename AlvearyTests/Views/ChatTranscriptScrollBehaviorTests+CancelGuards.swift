...
// through while rejecting the programmatic metric changes that happen to *look*
// like drags. Each regression test below pins one false-positive
...
extension ChatTranscriptScrollBehaviorTests {
    ...
    // bottom anchoring can set `offsetY` to a negative value. A programmatic
    // bottom request in that state causes a large offsetY swing (e.g. 0 → -377)
    // that looks like a user drag per `offsetDecreased && movedFurtherFromBottom`,
    // but the new `distanceFromBottom` lands within the near-bottom band (e.g.
    // 12pt). The pre-fix cancel fired in that case, flipping `isFollowing` to
    // false and flashing the jump-to-latest button mid-stream — user-visible
    // symptom: "the button briefly appeared towards the end" of a long streaming
    // response. The fix requires the new position to be clearly past near-bottom
    // before honoring the cancel signal.
