...
/// The cache in front of harness discovery. Its whole job is to keep thread creation off the
/// subprocess fan-out, without ever answering a project-scoped or post-install read from memory.
final class AgentHarnessDiscoveryCacheTests: XCTestCase {
    ...
    private actor ProbeCountingDiscovery: AgentHarnessDiscoveryService {
        ...
        private var answer: [AgentHarnessID: AgentHarnessStatus] = [:]
        ...
        func setAnswer(_ answer: [AgentHarnessID: AgentHarnessStatus]) {
            self.answer = answer
        }
    ...
    private func makeService(timeToLive: TimeInterval = 60) -> Fixture {
        ...
        let clock = TestClock()
        return Fixture(
            service: CachingAgentHarnessDiscoveryService(
                base: base,
    ...
    func testASecondReadInsideTheWindowDoesNotProbeAgain() async {
        ...
        _ = await fixture.service.harnessStatuses(projectURL: nil)
    ...
    func testAStaleReadIsAnsweredFromTheSnapshotAndRefreshesBehindIt() async {
        ...
        let stale = await fixture.service.harnessStatuses(projectURL: nil)
        ...
        await waitUntil("expected the refresh to replace the snapshot") {
            let statuses = await fixture.service.harnessStatuses(projectURL: nil)
    ...
    func testConcurrentReadsShareOneProbe() async {
        ...
        async let first = fixture.service.harnessStatuses(projectURL: nil)
        async let second = fixture.service.harnessStatuses(projectURL: nil)
        async let third = fixture.service.harnessStatuses(projectURL: nil)
    ...
    func testAProjectScopedReadAlwaysPassesThrough() async {
        ...
        _ = await fixture.service.harnessStatuses(projectURL: projectURL)
        _ = await fixture.service.harnessStatuses(projectURL: projectURL)
    ...
    func testAProjectScopedReadDoesNotSeedTheSharedCache() async {
        ...
        _ = await fixture.service.harnessStatuses(projectURL: URL(fileURLWithPath: "/tmp/project"))
        _ = await fixture.service.harnessStatuses(projectURL: nil)
    ...
    func testAProbeInvalidatedMidFlightIsNotCached() async {
        ...
        async let inFlight = fixture.service.harnessStatuses(projectURL: nil)
        try? await Task.sleep(for: .milliseconds(50))
        await fixture.service.invalidate()
        await fixture.base.release()
        _ = await inFlight
    ...
    func testExplicitRefreshPreservesTheLastSnapshotUntilItsReplacementIsReady() async {
        ...
        let refreshed = await fixture.service.harnessStatuses(projectURL: nil)
        let count = await fixture.base.allCallCount
    ...
    func testExplicitRefreshEscapesAStalledColdProbeAndRejectsItsLateAnswer() async {
        ...
        await fixture.base.setAnswer(Self.statuses(version: "1.0.0"))
        await fixture.base.hold()
        let oldRead = Task { await fixture.service.harnessStatuses(projectURL: nil) }
        ...
        // The old harness ignores cancellation until its own gate opens, then answers with its old snapshot.
        await fixture.base.release()
        await refresh.value
        let repaired = await read.value
        let obsolete = await oldRead.value
        let latest = await fixture.service.harnessStatuses(projectURL: nil)
    ...
    private static func statuses(version: String) -> [AgentHarnessID: AgentHarnessStatus] {
        [
            .claude: AgentHarnessStatus(
                harnessId: .claude,
                definition: ClaudeHarnessDefinition.definition,
