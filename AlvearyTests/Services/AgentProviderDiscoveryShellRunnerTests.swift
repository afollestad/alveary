...
final class AgentProviderDiscoveryShellRunnerTests: XCTestCase {
    func testDiscoveryBoundsCommandsWithoutChangingTheGeneralSDKRunner() async throws {
        ...
        let result = try await AgentProviderDiscoveryShellRunner(shellRunner: shell).run(command)
    ...
    func testDiscoveryRejectsOversizedOutputInsteadOfParsingATruncatedResult() async throws {
        let runner = AgentProviderDiscoveryShellRunner(shellRunner: DefaultShellRunner())
        for script in ["print 'A' x 300000;", "print STDERR 'A' x 300000;"] {
            do {
                ...
                XCTAssertTrue(error is AgentProviderDiscoveryShellError)
            }
        }
    }
    ...
    func testTimedOutDiscoveryCleansUpDescendantsAndTheNextProbeCanRun() async throws {
        ...
        let runner = AgentProviderDiscoveryShellRunner(
    ...
    func testClaudeAuthDiscoveryRecoversAfterAStalledCLIWithoutLeavingDescendants() async throws {
        ...
        let resolver = AgentCLIKit.DefaultAgentProviderExecutableResolver(detector: .init(shellRunner: runner))
