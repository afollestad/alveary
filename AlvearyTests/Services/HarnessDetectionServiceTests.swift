...
final class HarnessDetectionServiceTests: XCTestCase {
    func testUncheckedHarnessesRemainUncheckedUntilProbeRuns() async {
        ...
        let service = DefaultHarnessDetectionService(
            shell: shell,
            registry: DefaultHarnessRegistry(agentRegistry: DefaultAgentRegistry())
        )
    ...
    func testHarnessCommandResolvesThroughWhichBeforeVersionCheck() async throws {
        let shell = MockShellRunner()
        ...
        await service.checkHarness("claude")
    ...
    func testHarnessCommandResolvesThroughLoginShellWhenWhichMissesUserPath() async throws {
    ...
    func testHarnessCommandResolvesThroughFallbackDirectoryWhenShellPathMisses() async throws {
