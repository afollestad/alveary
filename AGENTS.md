...
- The budget covers app sources only, because `build.sh` builds just the app target. CI's Test step compiles the test target with the same flags and surfaces over-budget test code as plain warnings that fail nothing — `AppDelegateTests+ScheduledTasks.swift` has one today.
