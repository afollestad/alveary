...
- Thread rename is inline (Finder-style `TextField` swap in `SidebarThreadRow`), not a modal sheet. The row tracks an `editingThreadID` binding; only `RenameSheet` for conversations (in `ThreadDetailView`) remains sheet-based.
