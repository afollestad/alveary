...
- `TYPECHECK_BUDGET_MS=<ms>` fails the build when any function body or expression in this repo's own sources exceeds that type-check time, and prints the offenders. CI sets `3000`, plus `TYPECHECK_TEST_BUDGET_MS=7000` for test sources; every entry point reads `scripts/lib/typecheck-budget.sh`, whose comments cover the mechanism, exclusions, and CI noise. See the type-check budget bullet in `Alveary/Views/AGENTS.md`.
    ...
    - A CI-only over-budget report on a trivial test statement is attribution noise; do not restructure the statement. It lands in `AppDelegateTests`, which `scripts/lib/typecheck-budget.sh` exempts from the failure scan. Exempt a suite rather than raising `TYPECHECK_TEST_BUDGET_MS` — the noise drifts upward across toolchains, so a raise buys a run or two and then costs another red build, and enough of them leave the test target unable to catch a genuine multi-second expression. App sources have never shown the noise, so `build.sh` keeps the base budget.
