...
- **Keep full proposal forms in panes.** The shell owns chrome, the summary line, and measurement; feature bodies may offer focused edits such as the review summary.
...
- **Open scheduled-task forms in `ScheduledTaskEditorPane`.** `Alveary/Views/Scheduled/AGENTS.md` owns that routing.
...
`AppKitReviewProposalWidgetView` edits its verdict and top-level comment locally; confirming awaits GitHub. `Alveary/Views/PullRequests/Review/AGENTS.md` owns what the preview shows, what its two badges mean, and the one action a staged comment has.
...
- **Keep summary typing inside the native editor.** Prepare saved Markdown through the transcript bridge, create the editor only for Edit, and invalidate only its row when the capped height changes. `AppKitReviewProposalSummaryView` owns the editing session.
