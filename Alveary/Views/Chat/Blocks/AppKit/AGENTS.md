...
- **A link or thread card is the control, not a card with a button.** These contribute no body view — the summary, a detail line, and a trailing chevron are the whole widget — and the bubble owns hover, the pointing-hand cursor, and the press, because the row view spans the transcript's full width and would light up far outside the card. Pressability is exactly `HostToolWidgetEntry.openableTarget != nil`, so running and refused cards stay inert; the shell switches on that target to pick the handler. Its chevron matches the tool row's disclosure glyph (regular weight, `transcriptInlineToolRowColor`); keep any new transcript chevron on that treatment.
- **A card waiting on its pull request spins in the chevron's slot.** Opening one the thread no longer links costs a GitHub round trip, so `AppState.pendingPullRequestPaneLookup` — carrying the asking thread, so only that transcript reacts — swaps the chevron for `AppKitStatusIndicatorSpinner`. `AppKitHostToolWidgetDisclosureSlotView` holds both in one square and publishes its own `firstBaselineOffsetFromTop`: the header stack aligns on the first baseline and neither a symbol nor a ring has one, so bottom-aligning rides the glyph high and grows the row. The spinner is inserted only while waiting, so a resolved card owns no repeating animation.
