...
**Keep a rule here only when the code that would violate it is not the code that documents it.** The deferred selection task and `statusVersion` invalidation, `ThreadDetailView+DecisionAttention`'s read site, and `ThreadDetailConversationResolver`'s transient-fetch fallbacks each carry theirs.
