...
- Long static user and assistant bubbles share `LongMarkdownBubbleContent` for frame/clipping/fade controls. Keep Show more/less on `AppHeaderToggle`; do not add bubble-wide gestures, nested vertical `ScrollView`s, or AppKit clipping bridges.
- Long collapsed bubbles use `DeferredAppMarkdownText` so full markdown parsing can finish off the main actor. Keep the fixed collapsed height stable, and render only the bounded preview markdown synchronously until the full parsed document arrives.
- Static user and assistant bubbles should use `.geometryGroup()` on the rounded markdown bubble surface so the background chrome and rendered text move as one unit during transcript reflow. Do not replace this with a transaction that disables animations; disabling row animations makes tool expansion and transcript list movement fight each other.
