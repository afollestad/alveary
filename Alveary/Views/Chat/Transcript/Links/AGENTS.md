...
- Markdown links open through `ChatView+Transcript+AppKitBridge.swift`.
...
- Resolve schemeless URLs against `workingDirectory`; `NSWorkspace` no-ops relative paths without `file://`.
...
- File-mention chips in user bubbles reuse the same handler.
