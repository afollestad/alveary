...
- **Add every new row-configuration input to `ContentSignature`.** The bridge coordinator skips the whole row rebuild when the signature is unchanged, so a field it does not enumerate silently never installs or removes its rows. Closure-valued inputs contribute the values they resolve for the current items, the way `approvalSelections` does.
    - **An input a persisted row reads goes in `PersistedSignature`.** The coordinator rebuilds only transient rows when just `transientRows` changed, so a persisted input filed there never refreshes its row.
- **Memoize body-level item projections in `AppKitTranscriptPresentationCache`, keyed on `ChatItemGrouper.itemsRevision`.** A hit must return the same array storage so `[ChatItem] ==` stays O(1), and the key must read every observable input or SwiftUI stops tracking it.
...
- **Render only the newest thought line.** `appKitTranscriptLiveThoughtSummaryText` returns the last renderable line of the accumulated thought, not a join of every line. Live thought text accumulates for a whole turn and harnesses separate reasoning sections with a blank line, so joining grows the row without bound and reads as one run-on sentence. A touching emphasis boundary (`****`) also counts as a section break, so a harness that emits no separator still renders its newest section. The completed-thought row shares the same helper and collapses the same way.
