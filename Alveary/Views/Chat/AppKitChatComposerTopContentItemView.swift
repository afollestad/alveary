...
final class AppKitChatComposerTopContentItemView: NSView {
    ...
    func configure(_ item: AppKitChatComposerTopContentView.Item, goalDisplayElapsedSeconds: Int? = nil) {
        switch item {
        case .goalStatus(let configuration):
            configureGoalStatus(configuration, displayElapsedSeconds: goalDisplayElapsedSeconds)
    ...
    func refreshGoalMetadata(displayElapsedSeconds: Int?) {
        guard case .goalStatus(let configuration) = content else {
            return
        }
        let metadata = Self.goalMetadata(for: configuration, displayElapsedSeconds: displayElapsedSeconds)
        let wasHidden = metadataField.isHidden
        metadataField.stringValue = metadata
        metadataField.isHidden = metadata.isEmpty
        guard wasHidden != metadataField.isHidden else {
            return
        }
        invalidateIntrinsicContentSize()
        needsLayout = true
    ...
    private func configureGoalStatus(
        _ configuration: AppKitChatComposerTopContentView.GoalStatusConfiguration,
        displayElapsedSeconds: Int?
    ) {
        ...
        metadataField.isHidden = metadata.isEmpty
        let restartEnabled = configuration.onRestartTerminal != nil && configuration.isRestartTerminalEnabled
        actionButton.configure(
            title: "Restart",
            isEnabled: restartEnabled,
            tooltip: configuration.restartTerminalDisabledTooltip
        )
        actionButton.actionHandler = restartEnabled ? configuration.onRestartTerminal : nil
        actionButton.isHidden = configuration.onRestartTerminal == nil
    ...
    private static func goalMetadata(
        for configuration: AppKitChatComposerTopContentView.GoalStatusConfiguration,
        displayElapsedSeconds: Int?
    ) -> String {
        var parts: [String] = []
        if let elapsedSeconds = displayElapsedSeconds ?? configuration.snapshot.elapsedSeconds {
