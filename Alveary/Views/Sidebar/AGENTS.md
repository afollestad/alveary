...
- Thread names with inline code are rendered by `SidebarThreadTitleChips`, which segments the name and wraps each chip in `.fixedSize().frame(height: bodyLineHeight, alignment: .center)` so the chip's rounded background visually overflows into the row's vertical padding without inflating the row height. `testSidebarThreadRowChipAndPlainShareHeight` locks this in — if you change the chip rendering, keep the chip row's height matching a plain row.
