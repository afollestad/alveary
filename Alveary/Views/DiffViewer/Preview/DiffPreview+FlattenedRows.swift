...
struct FlattenedDiffPreview: View {
    ...
    /// prepared-row rebuild reuses what has already been measured.
    @State private var rowHeights = DiffPreviewRowHeights()
                ...
                DiffPreviewScrollContainer(
                    ...
                    LazyVStack(alignment: .leading, spacing: 0) {
                        ForEach(preparedRows.rows) { row in
                            ...
                            .modifier(DiffPreviewRowHeightReporter(
                                key: row.heightKey(collapsedFileIDs: preparedRows.collapsedFileIDs)
                            ))
                        }
                    }
                    .modifier(DiffPreviewContentHeightModifier(plan: preparedRows.heightPlan))
                    .padding(.top, contentTopInset)
                    .appExpansionAnimationOverride(value: collapsedFileIDs)
                    .diffPreviewMinimumContentWidthFrame()
                    .textSelection(.enabled)
                }
                .environment(rowHeights),
                proxy: proxy,
...
struct FlattenedDiffPreviewPreparedRows: Sendable {
    ...
    /// What the row stream will draw to, so the scroll view is told a height instead of taking
    /// `LazyVStack`'s estimate. See ``FlattenedDiffPreviewHeightPlan``.
    let heightPlan: FlattenedDiffPreviewHeightPlan
    /// The collapse set `heightPlan`'s keys were built with; the renderer keys its measurements
    /// with this same set so the two agree on which file headers are collapsed.
    let collapsedFileIDs: Set<String>
}
