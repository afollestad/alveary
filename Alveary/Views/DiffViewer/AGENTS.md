...
- **`contentTopInset` is opt-in and scrolls with the content.** The PR pane passes it so the Changes tab's first row starts at the same top inset as its sibling tabs; host-side top padding would clip scrolled rows at a fixed gap instead. The diff viewer keeps the default `0`.
...
- **Comment chrome is interaction-supplied.** Avatars render only when the interaction carries an `avatarLoader`, reactions render only for comments with a node id, and the thread footer (Reply / Resolve) appears only with an interaction plus a remote root comment — all `nil`/absent on the diff viewer's inert path. Resolved threads collapse to a header row; the manual-expansion `@State` resets whenever `thread.isResolved` changes. Author rows compose the shared `PullRequestCommentAuthorRow`, and pending and submitted comments share one three-dot `actionsMenu` — pending always offers Edit + Delete (against the local batch), submitted gates on `canEdit`/`canDelete`. Save enablement is draft emptiness only; there is no saving-in-progress state because remote edits apply optimistically.
- **Editing is inline; composing is a row.** `DiffCommentInteraction.editingRemoteCommentID`/`editingPendingAnchor` swap the matching comment's body for `DiffCommentEditorControls` (in `DiffPreview+CommentComposer.swift`, shared with `DiffCommentComposerRow`) in place, keeping the author row; the host must nil `DiffCommentAnnotations.composerAnchor` while an edit is active so the standalone composer row — reserved for new comments and replies — never doubles up below the thread.
- **Comment rows fit the viewport, not the diff width.** `DiffPreviewScrollContainer` publishes `\.diffPreviewViewportContentWidth`; thread and composer rows frame to it so composing never needs horizontal scrolling (they still travel with horizontal scroll — no pinning). The hover add-comment button overlays the gutter's marker column via `DiffGutterLayout` widths, and its hover tracking must sit *after* the overlay in the modifier chain or the button steals its own hover and flickers away.
