...
/// Arrow-key and Cmd+A handling for whichever of the pane's two top lists is showing.
///
/// **Every host must gate `isEnabled` on `\.keepAliveTabIsActive`.** This is an app-level
/// `NSEvent` hook that being hidden cannot silence, and it resolves its scroll view by *overlap* —
/// the pane keeps both mode contents mounted, so the two lists share a rect and an ungated hidden
/// monitor answers the visible list's arrow keys too.
struct DiffViewerTopListKeyboardMonitor: NSViewRepresentable {
    let isEnabled: Bool
    /// Returns true when the key was consumed. Hosts compute the destination row and request its
    /// scroll from inside this callback rather than reacting to a selection `onChange`: SwiftUI
    /// can coalesce selection publishes during fast key repeat, which drops rows.
