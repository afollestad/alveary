...
///
/// **Never give it a `minWidth`, and keep it text-only.** A minimum makes the
/// label inflexible, so it competes with the action container's fixed frame
/// instead of yielding to it — at 360pt with all three file actions that
/// clipped the close button away, the one control this header must never drop.
/// `ViewThatFits` does not rescue this: any candidate with a concrete width is
/// inflexible for the same reason. The accepted cost is a narrow band around
/// 360pt where the label truncates to a bare ellipsis.
...
/// Reserves a fixed width for whichever actions the mode offers, derived from
/// `ActionButtonMetrics.iconButtonDiameter`, and animates between those widths.
/// The branch label beside it is the header's one flexible child, so an
/// unanimated change would snap it to its new width.
