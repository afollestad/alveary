        ...
        ScrollViewReader { scrollProxy in
            List(files) { file in
                ...
                .background {
                    ...
                    SecondaryClickTarget(onSecondaryClick: { _ in
                        claimKeyboardFocus()
                        if !isSelected(file) {
            ...
            .overlay {
                // The loading branch stays ahead of the emptiness branches. `performRefresh`
                // publishes `files` and flips `isLoadingFiles` in the same main-actor tick, so no
                // interleaved frame can leak the clean state through — but only while this checks
                // loading first.
                if isLoading {
                    VStack(spacing: 10) {
                        ProgressView()
