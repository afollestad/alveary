...
- **Scope `/effort` and `/model` to what the current selection actually offers.** Enable, reserve, suggest, and intercept `/effort` only while the selected model advertises effort options, and `/model` only with two or more candidates. `/model`'s candidates mirror `reasoningConfiguration.modelGroups` exactly — every ready harness before initial setup, the active harness afterward — so a pre-startup `/model` can switch harness through the same callback the picker uses.
