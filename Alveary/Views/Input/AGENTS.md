...
- `/model` is a thread-state-scoped Alveary local command. Its candidates mirror `reasoningConfiguration.modelGroups` exactly — every ready provider before initial setup, the active provider afterward — so a pre-startup `/model` can switch provider through the same callback the picker uses. Enable, reserve, suggest, and intercept it only with two or more candidates.
