...
- **Every image here is keyed to one store.** `AppKitMarkdownAttributedStringBuilder` replaces loaded `AppMarkdownInlineImageAttribute` runs with baseline-aligned `NSTextAttachment`s, and `AppKitMarkdownImageBlockView` reads its bitmap and its display size from the same `AppMarkdownImageStore`; measurer, renderer, `AppKitMarkdownView`, and the block view must share that instance *and* its `imageBaseURL` or measured and drawn heights diverge. Tests must inject a stub store — the default `.shared` store loads over the network.
- **Only inline loads rebuild the document.** `AppKitMarkdownView` rebuilds on the store notification for a source its *inline* runs reference, because an alt-text run becoming an attachment re-lays the text. An image block instead swaps its own bitmap in place; routing it through a rebuild would tear down the loaded view and restart the load.
