...
- `OcticonImage` renders the vendored Primer Octicon assets as template images. Octicons are fixed-canvas artwork (16px or 24px variants) and do not size by font, so every call site passes its own glyph box; tint comes from the ambient `foregroundStyle` so a button style's hover/disabled treatment still applies. Vendor the 16px variant when the glyph sits beside SF Symbols — its strokes are drawn bolder for small sizes and match SF weight, while 24px artwork renders visibly thinner at the same frame (the toolbar's `FileDiffOcticon` was swapped for exactly this). Attribution for each vendored icon lives in `Alveary/Resources/PrimerOcticonsAttribution.txt` — add a new icon there when vendoring one.
- `AppKitAnchoredPopover` (applied via `appKitPopover(isPresented:preferredEdge:content:)`) is the popover to use when the content must take **keyboard** focus. SwiftUI's `.popover` cannot: AppKit attaches a popover window as a *child* of the presenting window and keeps `NSApp.keyWindow` on the parent, so the child's `isKeyWindow` merely mirrors the parent — `@FocusState`, `makeFirstResponder`, and `makeKey()` all report success while ⌘V and typing keep going to the parent's first responder (measured: the field showed a caret and selection but never received the paste). Owning the `NSPopover` is what makes the fix reachable, and the fix is `window.parent?.removeChildWindow(window)` **before** `makeKeyAndOrderFront`, one runloop turn after `show(relativeTo:)` — earlier and AppKit re-attaches underneath. Its anchor view is flipped on purpose: `NSPopover` resolves `preferredEdge` in the anchor's coordinate space, so an unflipped anchor opens `.maxY` *upward*. Keep it flipped so the edge reads in screen terms. Do not "fix" a non-typing popover by adding focus retries or key-claim loops; they cannot win against the child-window relationship. Live `NSPopover` host tests are banned (see `AlvearyTests/AGENTS.md`), so this path is verified by hand.
- `SecondaryClickTarget` reports right-clicks and control-clicks inside its own bounds through an AppKit local event monitor. SwiftUI has no secondary-click gesture and `contextMenu` only opens a menu, which is too late for a caller that must act on mouse-down (the diff viewer selects the clicked row before the menu opens) and wrong for a caller that wants something other than a menu (the toolbar's pull-request button opens a popover). Apply it as an `overlay`/`background` on the control that should respond. Two properties make placement safe: the monitor returns the event unchanged, and the view's `hitTest` returns nil — the bounds check lives in the monitor, so the view must never claim clicks itself. Without that override an `.overlay` placement silently eats the control's left-clicks (how the toolbar button first shipped dead).
...
- `AppScrollIndicatorLayout.interactiveTrailingClearance` (28) is the minimum distance from a scroll view's trailing edge to an **interactive** control's trailing edge. Overlay scrollers float over content and their grab region reaches past the visible 17pt lane — measured: controls 3pt clear of the lane still dropped clicks while 11pt clear never did — so anything closer silently loses clicks whenever the indicator shows. It is the dual of `appKitHorizontalOverflowScrollbarReserve`: that one reserves *space at the end of content* and so only fires for legacy scrollers; this one guards *hit-testing under a floating indicator* and so only matters for overlay ones.
    - **Keep the number repo-owned, and never derive it from `NSScroller.preferredScrollerStyle`.** The lane width has changed across macOS releases, so reading it at layout time desyncs snapshots between a developer's machine and CI's; reading the *preferred style* would be worse still, letting a user's "Show scroll bars" setting move layout. `AppScrollIndicatorLayoutTests` asserts the clearance still exceeds the real lane by the measured slop margin, so an OS widening the lane fails one fast test.
    - **Spend the clearance inside existing chrome, not as visible gaps.** Shrinking rows or floating a lone control inboard reads as misalignment (both were tried and rejected); instead deepen a card's interior trailing padding or pad a whole trailing column so controls land on one shared visual column. `.contentMargins(.trailing, …, for: .scrollContent)` insets *all* content and is likewise the wrong tool.
- **Every bar-backed pane footer goes through `contextualPaneFooterChrome()`.** The modifier owns the whole strip — horizontal insets, vertical padding, `.bar` fill, and the top hairline inset to match `ContextualPaneHeader`'s bottom one — and its vertical padding is a file-private constant reachable no other way, deliberately: three surfaces had hand-rolled that chrome to 16 / 12 / 10 with two of them drawing a full-bleed hairline before it was unified. Do not re-inline the padding, background, or overlay at a call site, and do not promote the constant. `ContextualPaneLayout.contentInsets(vertical:)` is *scroll content* only — reaching for it in a footer is what caused the drift. `SnapshotTests+PaneFooterChrome.swift` is the parity gate: it stacks a `ContextualPaneFooter` and the pull-request review footer in one baseline, so a divergence reads as a visible step.
