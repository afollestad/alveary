...
- `ActionIcon` (`ActionIcon.swift`) names a button glyph as `.system` or `.octicon`, and is the **one icon vocabulary shared with AppKit** — `ActionIcon+AppKit.swift` renders the same value for transcript buttons, so a concept keeps one glyph everywhere it appears (a pull-request action is the pull-request octicon in the Diff Viewer footer *and* in a transcript card). `ActionButtonLabel` pairs it with a title and is the required icon+text content for the shared prominent styles.
    - **A menu row's `Label` needs `.labelStyle(.titleAndIcon)`.** macOS menu rows default to a *title-only* label style, so a bare `Label(_:systemImage:)` silently renders as text — which is how the diff file list's Stage/Discard and two selected-item checkmarks shipped glyph-less. Octicons do render there, as `Label { Text(…) } icon: { OcticonImage(…) }` with that same style; `DiffBranchSelectionMenu`'s branch rows are the proof.
    - **Glyph boxes are shared constants, never call-site literals.** `ActionButtonMetrics.octiconGlyphSize` (17) for `.body`-sized prominent buttons, `.inlineOcticonGlyphSize` (12) for `.plain` `.font(.caption)` inline buttons. If a glyph reads wrong, move the constant so every button at that size follows.
- **`Octicon` (`Octicon.swift`) is the closed set of vendored glyphs; never pass an asset name as a `String`.** A missing asset draws nothing and raises no compile error, so `OcticonAssetTests` resolves every case and checks its canvas against the case-name suffix. `assetName` belongs only where the image is loaded, never in a signature.
...
- `KeepAliveTabContainer` hosts a tabbed pane's content so a visited tab survives deselection, preserving its scroll offsets and prepared row models; a `switch` over the selection rebuilds the incoming tab from scratch. The PR pane and the Diff Viewer's modes use it.
    - **Content that reaches input through AppKit must gate on `\.keepAliveTabIsActive`.** Hiding is a rendering concern — an `NSEvent` monitor or a first-responder claim in a mounted-but-hidden tab still fires. It defaults to `true` so content hosted outside a container behaves as active.
    - **A hidden tab's `onDisappear` no longer runs on a switch.** Cleanup that a deselected tab owes its next activation belongs on `onChange(of:)` of that environment value, with `onDisappear` kept for real teardown.
    - **Apply the container outside the tabs' `.equatable()`, never inside.** It hides by modifier, and pane tabs routinely compare equal across a selection change — the case where SwiftUI reuses rendered output.
    - **The container is deliberately not `Equatable`**, unlike the pane children around it: re-running its body is how a session write reaches the mounted tabs.
    - **A chip row driven by `TabChipButtonStyle` needs `.id(selection)`.** Keeping tabs mounted makes a switch structurally identical, so SwiftUI reuses the style's rendered output and the highlight sits a selection behind — the tearing-down `switch` had been forcing that repaint for free. Both adopters carry it; `KeepAliveTabContainerTests` clicks a real chip and switches back, which is the pass that catches it.
...
- `AppScrollIndicatorLayout.interactiveTrailingClearance` (28) is the minimum distance from a scroll view's trailing edge to an **interactive** control's trailing edge: the overlay scroller's grab region reaches past the visible 17pt lane — controls 3pt clear still dropped clicks while 11pt clear never did. Its dual `appKitHorizontalOverflowScrollbarReserve` reserves end-of-content space and only fires for legacy scrollers.
    ...
    - **It governs scroll-view interiors only.** A pane header, tab row, or footer has no scroller floating over it, so its chrome takes the glyph lane below instead.
- **A pane's chromeless trailing glyphs *center* on `ContextualPaneLayout.trailingGlyphAxis`, never right-align.** They are sparse and far apart vertically, so the eye reads each one's mass rather than a right rule; aligning ink edges instead scattered their centers across 4.75pt, because SF Symbols of one point size differ in width. The axis is the midline of a 12pt lane whose outer edge is `horizontalInset` — 12 being the widest of these glyphs, so the lane is the narrowest that holds them all.
    - **Take the lane, never a measured per-glyph constant.** `contextualPaneTrailingGlyphLane()` frames a bare row glyph; the `controlWidth:` overload pulls a wider chromeless control (a 30pt icon button) out until its frame center reaches the axis, so its hit target overhangs the inset — the point, not a defect, since these draw nothing until hover. Width stops mattering, which is what retired the old measured insets.
        - **A header reserving space for one must reserve to where it lands.** Subtract `trailingGlyphLaneOverhang(controlWidth:)`, or the reserve both widens the gap to the neighbouring control and, once the row overflows, pushes the laned one clean off the pane — `DiffViewerPaneMetrics.headerCloseButtonReservedWidth` is the worked example.
    ...
    - **Text stays right-aligned on the content column.** Timestamps and diff stats are not lane glyphs; a ragged right edge down a run of them reads worse than their centers wandering with string length.
...
- **`isBusy` says working; `disabled` says unavailable.** It swaps the glyph for `StatusIndicatorSpinner` in the glyph's own box (through `ActionButtonLabel`, so the width cannot move) and blocks both the primary side and the caret, while leaving the fill, border, and label at their enabled opacities — a greyed-out pill reads as "you cannot do this", not "this is running". A text-only split button gets no spinner; give one an icon before making it busy.
