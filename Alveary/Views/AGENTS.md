...
- **Do not claim composer focus by writing to the composer's `@FocusState` binding.** `ChatInputField` has no `.focused($isInputFocused)` anchor; the binding bridges into AppKit through a Representable parameter. Programmatic writes update storage but do not reliably invalidate descendants or claim AppKit first responder. AppKit-to-SwiftUI writes from `handleFocusChange` still publish focus changes. Body-time focus reads, such as `inlineSlashCommandHint`, must use the plain-binding mirror documented in `Alveary/Views/Components/TextInput/AGENTS.md`. New programmatic focus paths must route through `requestFirstResponder` or an equivalent AppKit-side claim, then let `handleFocusChange` backfill `@FocusState`.
