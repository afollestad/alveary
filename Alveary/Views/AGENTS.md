...
- **Do not claim composer focus by writing to a `@FocusState` binding.** The production composer routes programmatic focus through `requestFirstResponder` tokens and BlockInputKit focus APIs. Legacy SwiftUI `ChatInputField` still bridges focus for snapshots, but new production focus paths must use `ChatComposerFocusHandle` or the token-driven BlockInput path.
