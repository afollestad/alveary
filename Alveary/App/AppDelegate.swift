...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    override init() {
        ...
        self.notificationTapDelegate = Self.makeNotificationTapDelegate(dependencies: dependencies)
    ...
    private static func makeNotificationTapDelegate(dependencies: Dependencies) -> NotificationTapDelegate {
        NotificationTapDelegate(
            router: dependencies.notificationRouter,
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        dependencies.menuBarController.start()
        // App-scoped so the global capture shortcut survives the window closing.
        dependencies.appShotCoordinator.start(settingsService: dependencies.settingsService)
    ...
    /// The status item and the Dock icon are how the user gets back in, so closing the last window
    /// is a hide, not a quit. Agent runs, scheduled tasks, and the capture shortcut all keep going.
    ...
    func applicationWillTerminate(_ notification: Notification) {
        startupTask?.cancel()
        wakeRefreshTask?.cancel()
        dependencies.menuBarController.stop()
        dependencies.appShotCoordinator.stop()
