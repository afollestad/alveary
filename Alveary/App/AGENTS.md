...
- The top-right toolbar uses one custom SwiftUI `ToolbarItem` for project actions, terminal, diff viewer, and settings:
    ...
    - **Preserve spacing and inset.** Keep `PrimaryToolbarButtonGroup` at `HStack(spacing: 4)` with `8` points of horizontal inset and `4` points of vertical inset, matching the native toolbar capsule around 30-point controls.
    - **Anchor the trailing edge.** Use the destination group width with trailing alignment so the visible capsule animates leftward from its right edge and never keeps a persistent offset.
