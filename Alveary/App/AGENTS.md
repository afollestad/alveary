...
- The Diff Viewer toolbar button owns the visible changed-diff summary. Keep the green `+N` and red `-N` text inside that button instead of adding transcript or composer chrome for changed files; composer-height changes can invalidate lazy transcript row hit testing and leave stale bottom space.
