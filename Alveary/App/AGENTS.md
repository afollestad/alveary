...
- **Route context-scoped menu shortcuts through `FocusedValues+Alveary.swift`.** When a shortcut only applies while a specific view is mounted (e.g. ⌘T "New Conversation" requires a `ThreadDetailView`), add a `FocusedValueKey` whose `Value` is the action closure, publish it via `.focusedSceneValue(\.<name>, { ... })` from the owning view, and wrap the `Button` inside `AlvearyApp.commands` in a small `View` struct that reads `@FocusedValue`. `.disabled(action == nil)` keeps the menu item correctly greyed out when the publishing view is not in the focused scene — no `appState.selectedSidebarItem` inspection needed at the menu layer, and no duplication of the view's private logic (e.g. `ThreadDetailView.createConversation()`).
