...
- The Diff Viewer toolbar button owns the visible changed-diff summary:
    ...
    - **Avoid toolbar animations.** Render the current icon/count state directly for now. Do not animate toolbar button widths or keep outgoing toolbar labels mounted unless the whole toolbar interaction is intentionally redesigned and re-validated.
...
- The top-right toolbar should use ordinary toolbar item layout for project actions, terminal, diff viewer, and settings. Do not keep an empty custom-action region mounted, wrap the toolbar in a custom spacing container, or manually add inter-button spacing; AppKit's toolbar should own the spacing.
    - **Keep the terminal toolbar button native.** `TerminalToolbarButton` must remain a plain `Button` whose label is a SwiftUI `Label { Text(...) } icon: { ... }`; animate only the icon content so AppKit still controls whether toolbar text is shown.
    - **Preserve `ProgressView` toolbar grouping.** Keep the hidden base icon in the `Label` icon slot and overlay `ProgressView` in a fixed icon-sized frame; putting `ProgressView` directly in the icon slot makes AppKit render separate toolbar chrome.
