...
- **Keep conversation creation thread-scoped.** Pass the window's shared `NewConversationAction` directly to the title-adjacent `+`; reserve focused-value lookup for the menu. Keep `+` hidden before initial setup and visible-but-disabled while creation is blocked.
