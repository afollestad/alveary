...
- Any code path that removes a conversation must dismiss its banner and clear its unread flag *before* the SwiftData delete. That both dismisses any pending OS banner and captures the post-mark-read unread count for the chained `refreshBadgeCount()` task; skipping it leaves an orphaned banner and a stale dock badge until the next event. Use `NotificationManager.forgetConversations(in:)` for thread/project-level removals while the caller still has live models, `forgetConversations(withIDs:)` after async snapshotting when only conversation IDs are safe to carry across an `await`, and `markConversationRead(conversationId:)` directly when removing a single conversation inside a still-live thread (`ThreadDetailView.removeConversation`).
