...
- **`stdoutLimitBytes` is opt-in and defaults to unbounded, so a bounded read is the caller's job.** Any call whose output grows with repository or response size sets a limit *and* turns `ShellResult.stdoutWasTruncated` into an error — never parses the partial capture, which is a valid-looking prefix. `CLIGitService` (`GitError.outputTooLarge`) and `GitHubPullRequestsService` (`responseTooLarge`) are the models; each scope owns its own byte figures.
...
- **`DefaultExecutablePathResolver` caches resolved paths for the life of the process**, which is app-lifetime because `AppComponent.executablePathResolver` is the single shared instance. Resolving spawns `/usr/bin/which` and, on a miss, a login shell per candidate shell, and every `gh` call plus every harness re-check goes through it.
    - **Cache successes only.** Onboarding re-checks a dependency immediately after installing it and the post-wake sweep re-checks every harness, so a cached negative would report a binary that now exists as still missing.
...
- **Keep review workers grouped.** `.create` launches through macOS's `/usr/bin/perl`; hold registration through TERM/KILL teardown so clean shutdown also stops descendants. Abrupt-crash cleanup is not guaranteed.
