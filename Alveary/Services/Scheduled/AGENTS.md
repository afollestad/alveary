...
- **Scheduling's read surface is `list_scheduled_tasks` alone,** and it takes no arguments; an argument-taking read could be steered into probing. `list_projects` and `list_threads` belong to `Services/Threads/`, and `propose_scheduled_task` still directs the model to them for placement — safe because exposure is all-or-nothing per turn.
