...
- Keep this folder feature-neutral. A feature owns its catalog and handler in its own folder (`Services/Scheduled/`, `Services/Threads/`, `Services/PullRequests/`) and enrolls here.
...
- Enrolled today: `ScheduledTaskHostToolCatalog` (`Services/Scheduled/`), `ThreadHostToolCatalog` (`Services/Threads/`), and `PullRequestHostToolCatalog` (`Services/PullRequests/`). A tool one feature directs the model toward may live in another — `propose_scheduled_task` names `list_projects` and `list_threads` — which is safe only because exposure is all-or-nothing per turn.
