...
- Right-pane widths are independent persisted numeric settings for Diff, Skills, MCP, and Scheduled. They share the `320...960` policy and default to `380`; adding a width remains an additive decode with no schema bump.
- Pull Requests screen state (`pullRequestsSelectedTab`, `pullRequestsStatusFilters`, `pullRequestsRepositoryFilters`) persists as raw strings plus typed sets. Tab strings that no longer name a tab fall back at the restore site; unknown status strings drop out element-wise during decode.
