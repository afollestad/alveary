@preconcurrency import AppKit
import Carbon
import Foundation

/// An immutable physical keyboard chord used for event matching, display, and conflict checks.
struct PhysicalKeyboardShortcut: Codable, Hashable, Identifiable, Sendable {
    let keyCode: UInt16
    let modifiers: PhysicalKeyboardShortcutModifiers
    let keyEquivalent: String

    var id: String {
        "\(keyCode)-\(modifiers.rawValue)"
    }

    var displayString: String {
        let displayKey = keyCode == UInt16(kVK_Space) ? "Space" : keyEquivalent
        return modifiers.displayPrefix + displayKey
    }

    var modifierCount: Int {
        modifiers.count
    }

    var isModifierKey: Bool {
        Self.modifierKeyCodes.contains(keyCode)
    }

    var normalized: PhysicalKeyboardShortcut? {
        let normalizedModifiers = modifiers.normalized
        guard normalizedModifiers.count >= 2,
              !keyEquivalent.isEmpty else {
            return nil
        }
        return PhysicalKeyboardShortcut(
            keyCode: keyCode,
            modifiers: normalizedModifiers,
            keyEquivalent: keyEquivalent
        )
    }

    var voiceInputNormalized: PhysicalKeyboardShortcut? {
        guard !isModifierKey else { return nil }
        return normalized
    }

    func matches(event: NSEvent) -> Bool {
        matches(
            keyCode: UInt16(event.keyCode),
            modifiers: PhysicalKeyboardShortcutModifiers(event.modifierFlags)
        )
    }

    func matches(keyCode: UInt16, modifiers: PhysicalKeyboardShortcutModifiers) -> Bool {
        self.keyCode == keyCode && self.modifiers == modifiers.normalized
    }

    /// Key-up tracking intentionally ignores modifiers because users may release them first.
    func matchesRelease(keyCode: UInt16) -> Bool {
        self.keyCode == keyCode
    }

    static func recorded(
        from event: NSEvent,
        allowsModifierKey: Bool = false
    ) -> PhysicalKeyboardShortcut? {
        let keyCode = UInt16(event.keyCode)
        let modifiers = PhysicalKeyboardShortcutModifiers(event.modifierFlags)
        guard modifiers.containsAnyActivationModifier,
              allowsModifierKey || !modifierKeyCodes.contains(keyCode),
              let keyEquivalent = keyEquivalent(for: keyCode, event: event) else {
            return nil
        }
        return PhysicalKeyboardShortcut(
            keyCode: keyCode,
            modifiers: modifiers,
            keyEquivalent: keyEquivalent
        )
    }

    static let controlShiftSpace = PhysicalKeyboardShortcut(
        keyCode: UInt16(kVK_Space),
        modifiers: [.control, .shift],
        keyEquivalent: "Space"
    )

    static let controlCommandShiftSpace = PhysicalKeyboardShortcut(
        keyCode: UInt16(kVK_Space),
        modifiers: [.command, .control, .shift],
        keyEquivalent: "Space"
    )

    static func == (lhs: PhysicalKeyboardShortcut, rhs: PhysicalKeyboardShortcut) -> Bool {
        lhs.keyCode == rhs.keyCode && lhs.modifiers == rhs.modifiers
    }

    func hash(into hasher: inout Hasher) {
        hasher.combine(keyCode)
        hasher.combine(modifiers)
    }

    private static func keyEquivalent(for keyCode: UInt16, event: NSEvent) -> String? {
        if let symbol = specialKeySymbols[keyCode] {
            return symbol
        }
        guard let characters = event.charactersIgnoringModifiers?.trimmingCharacters(in: .whitespacesAndNewlines),
              !characters.isEmpty else {
            return nil
        }
        return characters.uppercased()
    }

    private static let modifierKeyCodes: Set<UInt16> = [
        UInt16(kVK_Command),
        UInt16(kVK_Shift),
        UInt16(kVK_CapsLock),
        UInt16(kVK_Option),
        UInt16(kVK_Control),
        UInt16(kVK_RightCommand),
        UInt16(kVK_RightShift),
        UInt16(kVK_RightOption),
        UInt16(kVK_RightControl),
        UInt16(kVK_Function)
    ]
}

struct PhysicalKeyboardShortcutModifiers: OptionSet, Codable, Hashable, Sendable {
    let rawValue: UInt8

    static let command = PhysicalKeyboardShortcutModifiers(rawValue: 1 << 0)
    static let control = PhysicalKeyboardShortcutModifiers(rawValue: 1 << 1)
    static let option = PhysicalKeyboardShortcutModifiers(rawValue: 1 << 2)
    static let shift = PhysicalKeyboardShortcutModifiers(rawValue: 1 << 3)

    init(rawValue: UInt8) {
        self.rawValue = rawValue
    }

    init(_ flags: NSEvent.ModifierFlags) {
        var result = PhysicalKeyboardShortcutModifiers()
        let normalized = flags.intersection(.deviceIndependentFlagsMask)
        if normalized.contains(.command) { result.insert(.command) }
        if normalized.contains(.control) { result.insert(.control) }
        if normalized.contains(.option) { result.insert(.option) }
        if normalized.contains(.shift) { result.insert(.shift) }
        self = result
    }

    var containsAnyActivationModifier: Bool {
        contains(.command) || contains(.control) || contains(.option)
    }

    var count: Int {
        var result = 0
        if contains(.command) { result += 1 }
        if contains(.control) { result += 1 }
        if contains(.option) { result += 1 }
        if contains(.shift) { result += 1 }
        return result
    }

    var normalized: PhysicalKeyboardShortcutModifiers {
        intersection([.command, .control, .option, .shift])
    }

    var displayPrefix: String {
        var result = ""
        if contains(.control) { result += "⌃" }
        if contains(.option) { result += "⌥" }
        if contains(.shift) { result += "⇧" }
        if contains(.command) { result += "⌘" }
        return result
    }

    var carbonFlags: UInt32 {
        var result: UInt32 = 0
        if contains(.command) { result |= UInt32(cmdKey) }
        if contains(.control) { result |= UInt32(controlKey) }
        if contains(.option) { result |= UInt32(optionKey) }
        if contains(.shift) { result |= UInt32(shiftKey) }
        return result
    }
}

enum PhysicalKeyboardShortcutAssignment: Equatable, Sendable {
    case appShot
    case voiceInput
}

enum PhysicalKeyboardShortcutValidation {
    static func message(
        for shortcut: PhysicalKeyboardShortcut,
        assignment: PhysicalKeyboardShortcutAssignment,
        appShotShortcut: PhysicalKeyboardShortcut?,
        voiceInputShortcut: PhysicalKeyboardShortcut?,
        displayString: String? = nil,
        hasEnabledSystemConflict: ((PhysicalKeyboardShortcut) -> Bool)? = nil
    ) -> String? {
        let displayText = displayString ?? shortcut.displayString
        if assignment == .appShot,
           knownSystemShortcutNames[shortcut] != nil || systemConflict(
               for: shortcut,
               override: hasEnabledSystemConflict
           ) {
            return "\(displayText) conflicts with a macOS keyboard shortcut."
        }
        if shortcut.modifierCount < 2 {
            return "Use at least two modifier keys."
        }
        if assignment == .voiceInput, shortcut.isModifierKey {
            return "Use a nonmodifier key."
        }
        if assignment == .voiceInput,
           shortcut.keyCode == UInt16(kVK_Escape) {
            return "Escape is reserved for canceling actions."
        }
        if assignment == .voiceInput,
           knownSystemShortcutNames[shortcut] != nil || systemConflict(
               for: shortcut,
               override: hasEnabledSystemConflict
           ) {
            return "\(displayText) conflicts with a macOS keyboard shortcut."
        }
        if let conflict = reservedShortcutNames[shortcut] {
            return "\(displayText) is already used by \(conflict)."
        }

        switch assignment {
        case .appShot:
            if shortcut == voiceInputShortcut {
                return "\(displayText) is already used by Voice Input."
            }
        case .voiceInput:
            if shortcut == appShotShortcut {
                return "\(displayText) is already used by App Shots."
            }
        }
        return nil
    }

    static func hasEnabledSystemShortcut(matching shortcut: PhysicalKeyboardShortcut) -> Bool {
        SystemKeyboardShortcutStore.hasEnabledShortcut(matching: shortcut)
    }

    private static func systemConflict(
        for shortcut: PhysicalKeyboardShortcut,
        override: ((PhysicalKeyboardShortcut) -> Bool)?
    ) -> Bool {
        if let override {
            return override(shortcut)
        }
        return hasEnabledSystemShortcut(matching: shortcut)
    }
}

private let specialKeySymbols: [UInt16: String] = [
    UInt16(kVK_Return): "↩",
    UInt16(kVK_Tab): "⇥",
    UInt16(kVK_Space): "␣",
    UInt16(kVK_Delete): "⌫",
    UInt16(kVK_Escape): "⎋",
    UInt16(kVK_Command): "⌘",
    UInt16(kVK_Shift): "⇧",
    UInt16(kVK_CapsLock): "⇪",
    UInt16(kVK_Option): "⌥",
    UInt16(kVK_Control): "⌃",
    UInt16(kVK_RightCommand): "⌘",
    UInt16(kVK_RightShift): "⇧",
    UInt16(kVK_RightOption): "⌥",
    UInt16(kVK_RightControl): "⌃",
    UInt16(kVK_F1): "F1",
    UInt16(kVK_F2): "F2",
    UInt16(kVK_F3): "F3",
    UInt16(kVK_F4): "F4",
    UInt16(kVK_F5): "F5",
    UInt16(kVK_F6): "F6",
    UInt16(kVK_F7): "F7",
    UInt16(kVK_F8): "F8",
    UInt16(kVK_F9): "F9",
    UInt16(kVK_F10): "F10",
    UInt16(kVK_F11): "F11",
    UInt16(kVK_F12): "F12",
    UInt16(kVK_Home): "↖",
    UInt16(kVK_PageUp): "⇞",
    UInt16(kVK_ForwardDelete): "⌦",
    UInt16(kVK_End): "↘",
    UInt16(kVK_PageDown): "⇟",
    UInt16(kVK_LeftArrow): "←",
    UInt16(kVK_RightArrow): "→",
    UInt16(kVK_DownArrow): "↓",
    UInt16(kVK_UpArrow): "↑"
]

private let knownSystemShortcutNames: [PhysicalKeyboardShortcut: String] = [
    .init(keyCode: UInt16(kVK_Space), modifiers: [.command], keyEquivalent: "Space"): "Spotlight",
    .init(keyCode: UInt16(kVK_Space), modifiers: [.command, .option], keyEquivalent: "Space"): "Finder search",
    .init(keyCode: UInt16(kVK_Space), modifiers: [.command, .control], keyEquivalent: "Space"): "Character Viewer",
    .init(keyCode: UInt16(kVK_Tab), modifiers: [.command], keyEquivalent: "⇥"): "application switcher",
    .init(keyCode: UInt16(kVK_Tab), modifiers: [.command, .shift], keyEquivalent: "⇥"): "application switcher",
    .init(keyCode: UInt16(kVK_ANSI_Grave), modifiers: [.command], keyEquivalent: "`"): "window switcher",
    .init(keyCode: UInt16(kVK_ANSI_Grave), modifiers: [.command, .shift], keyEquivalent: "`"): "window switcher",
    .init(keyCode: UInt16(kVK_Escape), modifiers: [.command, .option], keyEquivalent: "⎋"): "Force Quit",
    .init(keyCode: UInt16(kVK_Escape), modifiers: [.command, .option, .shift], keyEquivalent: "⎋"): "Force Quit",
    .init(keyCode: UInt16(kVK_ANSI_Q), modifiers: [.command, .control], keyEquivalent: "Q"): "screen lock",
    .init(keyCode: UInt16(kVK_ANSI_Q), modifiers: [.command, .shift], keyEquivalent: "Q"): "log out",
    .init(keyCode: UInt16(kVK_ANSI_Q), modifiers: [.command, .option, .shift], keyEquivalent: "Q"): "log out immediately",
    .init(keyCode: UInt16(kVK_ANSI_D), modifiers: [.command, .option], keyEquivalent: "D"): "Dock hide/show",
    .init(keyCode: UInt16(kVK_ANSI_3), modifiers: [.command, .shift], keyEquivalent: "3"): "screenshot",
    .init(keyCode: UInt16(kVK_ANSI_4), modifiers: [.command, .shift], keyEquivalent: "4"): "screenshot",
    .init(keyCode: UInt16(kVK_ANSI_5), modifiers: [.command, .shift], keyEquivalent: "5"): "screenshot",
    .init(keyCode: UInt16(kVK_ANSI_3), modifiers: [.command, .control, .shift], keyEquivalent: "3"): "screenshot to clipboard",
    .init(keyCode: UInt16(kVK_ANSI_4), modifiers: [.command, .control, .shift], keyEquivalent: "4"): "screenshot to clipboard",
    .init(keyCode: UInt16(kVK_ANSI_5), modifiers: [.command, .control, .shift], keyEquivalent: "5"): "screenshot to clipboard"
]

private let reservedShortcutNames: [PhysicalKeyboardShortcut: String] = [
    .init(keyCode: UInt16(kVK_ANSI_Q), modifiers: [.command], keyEquivalent: "Q"): "Quit",
    .init(keyCode: UInt16(kVK_ANSI_H), modifiers: [.command], keyEquivalent: "H"): "Hide",
    .init(keyCode: UInt16(kVK_ANSI_H), modifiers: [.command, .option], keyEquivalent: "H"): "Hide Others",
    .init(keyCode: UInt16(kVK_ANSI_M), modifiers: [.command], keyEquivalent: "M"): "Minimize",
    .init(keyCode: UInt16(kVK_ANSI_M), modifiers: [.command, .option], keyEquivalent: "M"): "Minimize All",
    .init(keyCode: UInt16(kVK_ANSI_A), modifiers: [.command], keyEquivalent: "A"): "Select All",
    .init(keyCode: UInt16(kVK_ANSI_C), modifiers: [.command], keyEquivalent: "C"): "Copy",
    .init(keyCode: UInt16(kVK_ANSI_X), modifiers: [.command], keyEquivalent: "X"): "Cut",
    .init(keyCode: UInt16(kVK_ANSI_V), modifiers: [.command], keyEquivalent: "V"): "Paste",
    .init(
        keyCode: UInt16(kVK_ANSI_V),
        modifiers: [.command, .option, .shift],
        keyEquivalent: "V"
    ): "Paste and Match Style",
    .init(keyCode: UInt16(kVK_ANSI_Z), modifiers: [.command], keyEquivalent: "Z"): "Undo",
    .init(keyCode: UInt16(kVK_ANSI_Z), modifiers: [.command, .shift], keyEquivalent: "Z"): "Redo",
    .init(keyCode: UInt16(kVK_ANSI_S), modifiers: [.command], keyEquivalent: "S"): "Save",
    .init(keyCode: UInt16(kVK_ANSI_S), modifiers: [.command, .shift], keyEquivalent: "S"): "Save As",
    .init(keyCode: UInt16(kVK_ANSI_P), modifiers: [.command], keyEquivalent: "P"): "Print",
    .init(keyCode: UInt16(kVK_ANSI_F), modifiers: [.command], keyEquivalent: "F"): "Find",
    .init(keyCode: UInt16(kVK_ANSI_G), modifiers: [.command, .shift], keyEquivalent: "G"): "Find Previous",
    .init(keyCode: UInt16(kVK_ANSI_F), modifiers: [.command, .control], keyEquivalent: "F"): "Full Screen",
    .init(keyCode: UInt16(kVK_ANSI_T), modifiers: [.command, .option], keyEquivalent: "T"): "Show or Hide Toolbar",
    .init(keyCode: UInt16(kVK_ANSI_S), modifiers: [.command, .control], keyEquivalent: "S"): "Show or Hide Sidebar",
    .init(keyCode: UInt16(kVK_ANSI_Slash), modifiers: [.command, .shift], keyEquivalent: "/"): "Help",
    .init(keyCode: UInt16(kVK_ANSI_O), modifiers: [.command], keyEquivalent: "O"): "Add Project",
    .init(keyCode: UInt16(kVK_ANSI_N), modifiers: [.command], keyEquivalent: "N"): "New Thread",
    .init(keyCode: UInt16(kVK_ANSI_T), modifiers: [.command], keyEquivalent: "T"): "New Conversation",
    .init(keyCode: UInt16(kVK_ANSI_W), modifiers: [.command], keyEquivalent: "W"): "Close Conversation",
    .init(keyCode: UInt16(kVK_ANSI_W), modifiers: [.command, .option], keyEquivalent: "W"): "Close All",
    .init(keyCode: UInt16(kVK_ANSI_Comma), modifiers: [.command], keyEquivalent: ","): "Settings",
    .init(keyCode: UInt16(kVK_ANSI_D), modifiers: [.command, .shift], keyEquivalent: "D"): "Toggle Diff Viewer",
    .init(keyCode: UInt16(kVK_ANSI_T), modifiers: [.command, .shift], keyEquivalent: "T"): "Toggle Terminal",
    .init(keyCode: UInt16(kVK_ANSI_1), modifiers: [.command], keyEquivalent: "1"): "Conversation Tab 1",
    .init(keyCode: UInt16(kVK_ANSI_2), modifiers: [.command], keyEquivalent: "2"): "Conversation Tab 2",
    .init(keyCode: UInt16(kVK_ANSI_3), modifiers: [.command], keyEquivalent: "3"): "Conversation Tab 3",
    .init(keyCode: UInt16(kVK_ANSI_4), modifiers: [.command], keyEquivalent: "4"): "Conversation Tab 4",
    .init(keyCode: UInt16(kVK_ANSI_5), modifiers: [.command], keyEquivalent: "5"): "Conversation Tab 5",
    .init(keyCode: UInt16(kVK_ANSI_6), modifiers: [.command], keyEquivalent: "6"): "Conversation Tab 6",
    .init(keyCode: UInt16(kVK_ANSI_7), modifiers: [.command], keyEquivalent: "7"): "Conversation Tab 7",
    .init(keyCode: UInt16(kVK_ANSI_8), modifiers: [.command], keyEquivalent: "8"): "Conversation Tab 8",
    .init(keyCode: UInt16(kVK_ANSI_9), modifiers: [.command], keyEquivalent: "9"): "Conversation Tab 9"
]

private enum SystemKeyboardShortcutStore {
    static func hasEnabledShortcut(matching shortcut: PhysicalKeyboardShortcut) -> Bool {
        guard let domain = UserDefaults.standard.persistentDomain(forName: "com.apple.symbolichotkeys"),
              let hotKeys = domain["AppleSymbolicHotKeys"] as? [String: Any] else {
            return false
        }

        return hotKeys.values.contains { value in
            guard let dictionary = value as? [String: Any],
                  boolValue(dictionary["enabled"]) == true,
                  let shortcutValue = dictionary["value"] as? [String: Any],
                  let parameters = shortcutValue["parameters"] as? [Any],
                  parameters.count >= 3,
                  let keyCode = uint16Value(parameters[1]),
                  let modifierRawValue = uintValue(parameters[2]) else {
                return false
            }
            let modifiers = PhysicalKeyboardShortcutModifiers(NSEvent.ModifierFlags(rawValue: modifierRawValue)).normalized
            return shortcut.matches(keyCode: keyCode, modifiers: modifiers)
        }
    }

    private static func boolValue(_ value: Any?) -> Bool? {
        if let value = value as? Bool {
            return value
        }
        return (value as? NSNumber)?.boolValue
    }

    private static func uint16Value(_ value: Any?) -> UInt16? {
        guard let value = uintValue(value),
              value <= UInt(UInt16.max) else {
            return nil
        }
        return UInt16(value)
    }

    private static func uintValue(_ value: Any?) -> UInt? {
        if let value = value as? UInt {
            return value
        }
        if let value = value as? Int {
            return value >= 0 ? UInt(value) : nil
        }
        return (value as? NSNumber)?.uintValue
    }
}
