/// Packaged default for the Handoff tab's `Default session handoff prompt`.
///
...
enum SessionHandoffPromptDefaults {
    ...
    The goal is not to summarize the entire conversation. The goal is to preserve only the accurate and valuable context that \#
    will help the next session make progress faster while minimizing the initial context window.
    ...
    4. Omit instructions, workflow rules, or project conventions that the next agent can infer from existing `AGENTS.md` \#
    context in the target workspace.
    5. Produce a prompt that is ready to send as the first user message in a fresh continuation session.

    If the user already said what the next session should do, do not ask again. If the goal is still ambiguous after reading \#
    the conversation, infer the most likely next goal from the latest user request and active work.
    ...
    Do not start the prompt with meta-commentary like "Continue this work in a fresh session" - the prompt is already going \#
    into a new session, so that framing is redundant. Jump straight into the substance.
