...
- Runtime-ready harness status and model options are exposed through `AgentCLIKit.AgentHarnessDiscoveryService`; `AgentRegistry` remains Alveary's static metadata source for install guidance, sign-in commands, extra args, skills, and MCP integration.
