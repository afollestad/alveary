...
App-scoped pull-request services live here; collective runs are owned by `Collective/AGENTS.md`, host tools by `HostTools/`, and the GitHub client by `GitHub/`. Nothing under `GitHub/` may reach host tools, linking, SwiftData, or MCP.
...
- **Launch review tasks integration-isolated; never widen it.** `Kind.integrationIsolation` persists on the thread so every relaunch requests it, and `makeSpawnConfig` narrows it through `HarnessFeaturePolicy.launchIsolation`. A harness connector, plugin, or shell `gh` spends the user's shared GitHub quota outside `GitHubPullRequestsService`, where nothing sees or paces it.
