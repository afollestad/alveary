...
- **Nothing may move in front of `start`'s return.** Linking used to precede it and put a `gh api graphql` round trip between the footer click and the sidebar selection.
...
`+Workspace.swift` resolves where an `.addressFeedback` thread runs; its doc comment owns the ordering and the degrade-never-refuse rule. These are the constraints behind them.
