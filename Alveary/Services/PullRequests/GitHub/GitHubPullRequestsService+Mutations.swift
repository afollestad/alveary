...
extension GitHubPullRequestsService {
    ...
    func replyToReviewComment(_ id: PullRequestIdentifier, commentID: Int, body: String) async throws {
        let ghExecutable = try await resolveGitHubCLI()
        // A mutation: never retried. The pull number is required here: the shorter
        // `pulls/comments/{id}/replies` 404s, even though that form works for
        // GET/PATCH/DELETE on the same comment.
        let result = try await runGitHubCLI(
            executable: ghExecutable,
            args: [
    ...
    func requestReview(_ id: PullRequestIdentifier, reviewerLogin: String) async throws {
        let ghExecutable = try await resolveGitHubCLI()
        // A mutation: never retried. This endpoint accepts user logins only, which
        // is why `makeReviewers` withholds `canReRequest` from app and bot
        // reviewers (Copilot, Codex) rather than offering an action that 422s.
        let result = try await runGitHubCLI(
            executable: ghExecutable,
            args: [
    ...
    /// Reactions are GraphQL and address their subject by node `id`, not by a REST comment id —
    /// the pull request itself, issue comments, reviews, and review-thread comments all take them,
    /// so one pair of mutations covers every reactable surface rather than one endpoint per kind.
