...
extension PullRequestHostToolService {
    /// Opens a review submission for the user to confirm. Nothing reaches GitHub here.
    ///
    /// Submitting a review publishes a verdict and every pending comment attached to it, and GitHub
...
private extension PullRequestHostToolService {
    ...
    static func validate(
        ...
        case .comment:
            guard request.body != nil || detail.pendingCommentCount > 0 else {
    ...
    static func pendingMessage(
        ...
        if pendingCommentCount > 0 {
            message += ", publishing \(pendingCommentCount) pending review comment(s)"
        }
