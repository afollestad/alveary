...
- Use shared detection services for post-install verification. `gh` goes through `GitHubCLIService`; agent CLIs go through `HarnessDetectionService` and `AgentRegistry` metadata.
