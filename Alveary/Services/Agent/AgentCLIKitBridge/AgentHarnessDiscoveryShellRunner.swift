...
/// Kept separate from `AgentCLIKitShellRunnerAdapter` so harness turns and other SDK commands retain their existing budgets.
struct AgentHarnessDiscoveryShellRunner: AgentCLIKit.ShellRunning {
    ...
    func run(_ command: AgentCLIKit.ShellCommand) async throws -> AgentCLIKit.ShellCommandResult {
        ...
        guard !result.stdoutWasTruncated, !result.stderrWasTruncated else {
            throw AgentHarnessDiscoveryShellError.outputTooLarge
        }
...
enum AgentHarnessDiscoveryShellError: Error {
    case outputTooLarge
}
