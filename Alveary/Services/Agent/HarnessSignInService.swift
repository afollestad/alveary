...
/// Runs a harness's CLI sign-in in a live terminal tab and refreshes harness readiness afterwards.
///
...
@MainActor
@Observable
final class HarnessSignInService {
    /// The harness whose sign-in has not yet been confirmed, if any.
    private(set) var pendingHarnessID: String?

    private let agentRegistry: AgentRegistry
    private let discoveryService: CachingAgentHarnessDiscoveryService
    ...
    init(
        ...
        discoveryService: CachingAgentHarnessDiscoveryService,
    ...
    /// The sign-in command for a harness, or `nil` when the registry defines none.
    ///
    /// Callers gate their Sign In affordance on this rather than assuming every harness has one.
    func signInCommand(for harnessID: String) -> String? {
        agentRegistry.agent(for: harnessID)?.signInCommand
    }

    /// Opens a terminal tab running the harness's sign-in command.
    ///
    /// Returns `false` when the harness defines no sign-in command, so a caller that reached here
    /// anyway opens no empty tab.
    @discardableResult
    func startSignIn(harnessID: String, terminalManager: TerminalManager) -> Bool {
        guard let agent = agentRegistry.agent(for: harnessID),
    ...
    func handleRunningProjectActionSessionIDsChange(_ runningSessionIDs: Set<UUID>) -> Task<Void, Never>? {
        ...
        return refreshHarnessReadiness()
    }
    ...
    /// would put that fan-out on every activation for the rest of the session. The Harnesses settings
    ...
    func handleAppDidBecomeActive() -> Task<Void, Never>? {
        guard pendingHarnessID != nil, pendingSessionID == nil else {
            return nil
        }
        return refreshHarnessReadiness(stopsTrackingRegardless: true)
    }
}

private extension HarnessSignInService {
    /// Discards the cached snapshot, rebuilds it, and stops tracking once the harness reports ready.
    ...
    func refreshHarnessReadiness(stopsTrackingRegardless: Bool = false) -> Task<Void, Never>? {
        guard let harnessID = pendingHarnessID else {
            return nil
        }
        if stopsTrackingRegardless {
            pendingHarnessID = nil
        }
        return Task { @MainActor [weak self] in
            ...
            let statuses = await discoveryService.harnessStatuses(projectURL: nil)
            guard pendingHarnessID == harnessID,
                  statuses.values.first(where: { $0.harnessId.rawValue == harnessID })?.isSetupReady == true else {
                return
            }
            pendingHarnessID = nil
        }
