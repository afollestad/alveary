...
- `AgentsManager.suspendRuntime()` is non-destructive terminal cleanup. Suspend only idle, non-waiting runtimes, preserving the provider session, Alveary binding, conversation state, and reusable approvals so the next manual turn resumes. Call `isRuntimeSuspended(conversationId:)` before releasing terminal lifecycle ownership, because provider status can briefly lag the transcript boundary.
