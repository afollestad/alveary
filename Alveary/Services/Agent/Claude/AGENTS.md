...
- If the legacy direct Claude adapter remains reachable, keep its context-compaction stdout parsing aligned with AgentCLIKit so `status`, `compact_result`, `compact_error`, and compact metadata produce matching transcript lifecycle events.
