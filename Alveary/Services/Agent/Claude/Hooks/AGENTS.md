...
- `ToolApprovalRequest` owns tool-specific user-facing approval metadata:
    ...
    - **Use `ToolApprovalPromptCopy` for exceptional transcript wording.** If a tool needs non-generic prompt text or action labels, such as `ExitPlanMode` asking whether the user is ready to leave plan mode and treating denial as "keep planning", add that copy on the request instead of branching inside transcript approval views.
