...
- Alveary host MCP tools (`mcp__alveary_host__*`) render as `.hostToolWidget` blocks only when they need their own affordances — today that is `propose_scheduled_task`, which carries a confirmation decision. `list_scheduled_tasks` stays an ordinary tool row and customizes just its expanded detail; prefer that unless the tool genuinely needs block-level chrome:
    - **Extend through `HostToolTranscriptCatalog`.** A new host tool gets custom transcript UI by adding a descriptor, a `HostToolWidgetContent` case with its parser, and an AppKit body view. Routing, markers, and row dispatch need no edits.
    - **Do not assume structured tool output.** Claude emits `JSON.stringify(structuredContent)` as the host tool's result text; Codex emits the plain-text fallback instead. A result that will not parse is still a success — only an explicit error is a failure — and the widget then resolves its proposal by conversation (a conversation holds one at a time) rather than by an id it never received.
    - **Match both reported tool-name shapes.** Providers disagree: Claude persists `mcp__alveary_host__<tool>` while Codex persists the bare `<tool>`, and both reach the transcript. Descriptors answer to either through `matches(toolName:)`, which delegates to `AlvearyHostToolCatalog.matches` (`Services/HostMCP/AGENTS.md` owns the mechanism); matching only the qualified name silently drops the widget back to a generic tool row.
    - **Route root-level calls only.** `handleHostToolWidgetCallIfNeeded` skips the tool classifier the way `TodoWrite` and `AskUserQuestion` do, but declines nested (`parentToolUseId != nil`) calls and any payload its parser cannot read, so those keep the generic tool row.
    - **Patch on result, never swallow it.** Unlike prompt and task-list tools, a widget's payload lives in its *result*; `handleToolResult` applies it to the rendered widget. A result that lands before its call is consumed when the call arrives.
    - **Resolve outcomes through `host_tool_outcome` markers.** Hidden rows carrying the feature's correlation key in `toolId`; unmatched markers cache by key until their widget renders. Markers never render and stay out of restore summaries (the summary builders are allowlists). A marker that matches no widget key pairs with the oldest marker-awaiting widget (running or applied, no outcome) that targets the definition the marker names — an immediate action's marker persists between the call and its result, and the definition match keeps a proposal's resolution off unrelated running actions — before falling back to the newest unresolved proposal; the result patch must carry an already-attached outcome forward instead of rebuilding it away.
