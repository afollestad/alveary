...
/// `HostToolWidgetContent` case, and adding its parser. Transcript grouping, outcome
/// markers, and row dispatch stay untouched. An AppKit body view is needed only for a
/// feature with its own controls — the link and thread cards render as bare bubbles.
