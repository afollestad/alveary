...
/// Claude harness setup, and the auth probe its readiness gate runs on.
///
...
extension AppComponent {
    ...
    var agentCLIKitHarnessSetup: AgentCLIKit.ClaudeHarnessSetup {
        return shared {
            demoClaudeHarnessSetup ?? AgentCLIKit.ClaudeHarnessSetup(
        ...
        return shared {
            AgentCLIKit.ClaudeAuthProbe(
                ...
                executablePath: { [executablePathResolver] in
                    guard let executableName = AgentCLIKit.ClaudeHarnessDefinition.definition.executableNames.first else {
                        return nil
                    }
    ...
    var harnessSignInService: HarnessSignInService {
        return shared {
            HarnessSignInService(
                agentRegistry: agentRegistry,
                discoveryService: cachedAgentHarnessDiscoveryService,
