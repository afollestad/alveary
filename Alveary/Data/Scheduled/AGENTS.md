...
**Keep a rule here only when the code that would violate it is not the code that documents it.** Mechanism whose only reader is its own file belongs in a doc comment: `ScheduledTaskProposalPlacement`'s create and edit defaults, `ScheduledTaskProposalWorkspace`'s deliberately absent run location and its grant-replacement semantics, and `ScheduledTaskProposalReceipt`'s legacy-nil fields each carry theirs.
...
- **Preserve optional exact target IDs through drafts, edits, and run snapshots.** A missing ID retains legacy main-conversation routing; an explicit ID never falls back. Leave frozen migration bridge models unchanged.
- **Keep target loss atomic with lifecycle commits.** `Alveary/Services/ScheduledTaskTargetDetachment.swift` pauses exact callbacks and converts legacy targets to reuse. `Alveary/ViewModels/Sidebar/AGENTS.md` owns the calling commits.
