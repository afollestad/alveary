...
- **`PullRequestReviewProposalLookup` is the reverse query for proposals**, as `PullRequestLinkedOwnerLookup` is for links; it skips an undecodable envelope rather than throwing, because one unreadable conversation must not hide every other pull request's proposal from a caller surveying all of them. The single-conversation read still throws.
