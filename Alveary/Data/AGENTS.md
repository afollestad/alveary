...
- Thread and main-conversation naming rules:
    ...
    - **Treat provider-generated names as automatic names.** Hidden provider metadata may replace `AgentThread.name` while `hasCustomName == false`, but it must keep `hasCustomName` false and never overwrite a manual rename.
