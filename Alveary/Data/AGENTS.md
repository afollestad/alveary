...
- **`Conversation.lastTurnFailedAt` is the durable half of `ThreadStatus.error`.** Written by the terminal-boundary writer on `ConversationState`, and before a first turn by `recordUnattendedStartFailure()` and `recordLaunchFailure`; cleared by `markVisibleTurnStarted()` and again before `sendReserved` dispatches or initial setup spawns. A turn-start path that skips that clear leaves a stale failure outranking a live spinner.
