...
extension ConversationViewModel {
    ...
    /// Filters the *delayed* fallout that lands after the denial call has already returned.
    ///
    /// Narrower than `shouldSuppressPromptDismissalEvent` on purpose, because by this point the user
    /// may have sent something new and its events must reach them. Suppression therefore ends the
    /// moment a replacement turn starts, or as soon as an event after a new outbound turn is not
    /// recognizably dismissal fallout.
