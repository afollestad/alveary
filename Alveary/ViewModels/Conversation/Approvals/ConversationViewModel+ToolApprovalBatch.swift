...
/// Discovery of the sibling approvals one decision should resolve alongside the row the user acted on.
///
/// Claude emits parallel tool calls as one batch but invokes their hooks one at a time, so answering
/// the visible row must carry the rest of its family or the composer stays blocked on prompts the
/// user already answered in spirit. Membership is decided from the persisted transcript rather than
/// from live state, because `tool_deferred` ends the local turn before the delayed siblings arrive —
/// `turnState.isActive` is false by then and is not a usable signal here.
extension ConversationViewModel {
    /// The other approvals in this one's batch, or empty when it stands alone.
    ///
    /// Returns both already-persisted approval rows and tool calls whose own hook has not fired yet,
    /// since a batch resolution has to cover the ones still in flight.
