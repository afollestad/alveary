...
extension ConversationViewModel {
    ...
    ///
    /// On failure the approval returns to `.pending` so the row stays actionable, and the staged
    /// plan follow-up and revision guidance are cleared rather than left keyed to a resolution that
    /// never happened.
    ...
    /// runtime reports. Treating it as live would skip spawn preparation and leave the old
    /// subscription pointed at a process that no longer exists.
...
private extension ConversationViewModel {
    ...
    /// Gets the conversation ready for a resume, draining persistence before the cursor is reset.
    ///
    /// `flushPendingSaveIfNeeded()` must complete *before* `resetSubscriptionTrackingForToolApprovalResume()`
    /// zeroes `lastPersistedEventIndex`. A debounced save still in flight — including one holding a
    /// staged plan follow-up — would otherwise be acknowledged against a cursor that no longer
    /// describes it, and its events would replay or vanish. The flush runs on both paths, because a
    /// live resolution can also have queued saves behind it.
