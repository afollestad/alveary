...
- `DiffViewerViewModel` coordinates routing, watchers, contextual actions, and store delegation. Keep durable Git-backed file/stat/diff state in `DiffWorkspaceStore`.
    ...
    - **Honor switch scope.** `switchToTarget(_:scope:)` with `.toolbarStatsOnly` loads status and toolbar stats but skips selected-diff work; callers use it while the diff pane is hidden. A later same-target `.full` switch must upgrade via `needsFullPaneRefresh` instead of deduping, and the flag is set eagerly at switch time so a pane reveal racing the scheduled refresh still upgrades.
