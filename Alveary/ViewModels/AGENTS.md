...
- Pinned sidebar ordering is mode-explicit: Task threads use the Task drag domain and stay independently pinned even when backed by a pinned Project; Project pin normalization may absorb only Project-mode child threads. Unpinned Task threads may enter the Pinned order via the `.unpinnedTask` drag item, which sets `isPinned` and a dense `pinnedSortOrder` at commit; `.pinnedTask` sources still require an actually-pinned Task. A `.tasks`-section drop is an unpin, not a reorder — `commitSidebarDrop` delegates it to `setThreadPinned(_:isPinned: false)`, which owns the scheduled-attachment guard.
