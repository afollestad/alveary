...
- Keep live tool-approval decisions terminal-aware:
  ...
  - **Clear denied plan exits immediately.** A denied or dismissed `ExitPlanMode` approval is terminal for the confirmation UI; clear the pending approval, end the local turn, and only drain a custom follow-up when the user provided one.
