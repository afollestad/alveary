...
- Thread removal routes through `SidebarViewModel` lifecycle methods so runtime teardown, notification cleanup, harness-native session retirement (archive on archive, delete on delete — see `Alveary/Services/Agent/AGENTS.md`), worktree cleanup, and branch cleanup stay coordinated. Views needing to delete a thread — including project-trust denial flows — receive a focused delete closure instead of calling `ModelContext.delete(_:)` on `AgentThread`.
...
- **Quiesce exact callback runs before removing their target; preserve legacy live-run refusals.** Archive, thread delete, and project delete re-resolve after stopping, fence racing claims, and call `ScheduledTaskTargetDetachment.detachTargets(of:)` inside their lifecycle save.
    - **Route every cleanup refusal through `ThreadLifecycleService.threadCleanupError(for:)`** so action guards, sidebar tooltips, and host tools agree; the hover pill bypasses `requestArchive`/`requestDelete`. Pending review proposals alone do not block cleanup.
    - Publish through `postScheduledTasksDetached` only after that save commits.
    - Keep workspace retargeting guarded by `scheduledTaskAttachmentError(for:)`; `ThreadDetailConversationDeletion` separately protects legacy main targets and quiesces exact tab callbacks.
...
- `SidebarViewModel.moveTaskIntoProject(_:projectID:)` owns the sidebar's Task-to-Project drop. It changes placement and grants folder access; it never changes mode:
    ...
    - **Suspend, never destroy.** A running process was launched without the new root, so suspend it; the working directory is unchanged, so the harness resumes its session next turn. `kill`/`destroyRuntime` would discard the session record and lose the conversation.
    ...
    - **Trust does not apply.** Harness trust is a working-directory concept; granted roots are never auto-trusted.
